
import snowflake.connector
import os
import uuid
from datetime import datetime
from dotenv import load_dotenv
from groq import Groq

load_dotenv()

groq_client = Groq(api_key=os.getenv("GROQ_API_KEY"))

def get_snowflake_connection():
    return snowflake.connector.connect(
        account=os.getenv("SNOWFLAKE_ACCOUNT"),
        user=os.getenv("SNOWFLAKE_USER"),
        password=os.getenv("SNOWFLAKE_PASSWORD"),
        database=os.getenv("SNOWFLAKE_DATABASE"),
        warehouse=os.getenv("SNOWFLAKE_WAREHOUSE"),
        schema=os.getenv("SNOWFLAKE_SCHEMA")
    )

def run_quality_checks(bronze_table: str) -> dict:
    run_id = str(uuid.uuid4())[:8]
    issues_found = []
    bad_records = 0

    conn = get_snowflake_connection()
    cursor = conn.cursor()

    cursor.execute(f"SELECT COUNT(*) FROM {bronze_table}")
    total_records = cursor.fetchone()[0]
    print(f"Total records: {total_records}")

    cursor.execute(f"""
        SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_NAME = '{bronze_table}'
        AND TABLE_SCHEMA = 'PUBLIC'
        ORDER BY ORDINAL_POSITION
    """)
    all_cols = [r[0] for r in cursor.fetchall()]
    columns = [c for c in all_cols if not c.startswith('_')]
    print(f"Columns to check: {columns}")

    # CHECK 1 - NULL values
    for col in columns:
        cursor.execute(f"SELECT COUNT(*) FROM {bronze_table} WHERE {col} IS NULL")
        n = cursor.fetchone()[0]
        if n > 0:
            pct = round(n/total_records*100, 1)
            issues_found.append({
                "issue_type": "NULL_VALUE",
                "column": col,
                "count": n,
                "percentage": pct,
                "severity": "HIGH",
                "description": f"{n} NULL values ({pct}%) in {col}"
            })
            bad_records += n
            print(f"  NULL in {col}: {n}")

    # CHECK 2 - Negative numbers
    numeric_kw = ['quantity', 'amount', 'price', 'cost', 'total', 'stock', 'unit', 'reorder', 'hand']
    for col in columns:
        if any(k in col.lower() for k in numeric_kw):
            try:
                cursor.execute(f"SELECT COUNT(*) FROM {bronze_table} WHERE {col} < 0")
                n = cursor.fetchone()[0]
                if n > 0:
                    issues_found.append({
                        "issue_type": "NEGATIVE_VALUE",
                        "column": col,
                        "count": n,
                        "percentage": round(n/total_records*100, 1),
                        "severity": "HIGH",
                        "description": f"{n} negative values in {col}"
                    })
                    bad_records += n
                    print(f"  NEGATIVE in {col}: {n}")
            except:
                pass

    # CHECK 3 - Future dates
    date_kw = ['date', 'time', 'created', 'updated']
    for col in columns:
        if any(k in col.lower() for k in date_kw):
            try:
                cursor.execute(f"""
                    SELECT COUNT(*) FROM {bronze_table}
                    WHERE {col} > CURRENT_TIMESTAMP()
                    AND {col} IS NOT NULL
                """)
                n = cursor.fetchone()[0]
                if n > 0:
                    issues_found.append({
                        "issue_type": "FUTURE_DATE",
                        "column": col,
                        "count": n,
                        "percentage": round(n/total_records*100, 1),
                        "severity": "MEDIUM",
                        "description": f"{n} future dates in {col}"
                    })
                    bad_records += n
                    print(f"  FUTURE DATE in {col}: {n}")
            except:
                pass

    # CHECK 4 - Duplicates
    if columns:
        pk = columns[0]
        cursor.execute(f"""
            SELECT COUNT(*) FROM (
                SELECT {pk}, COUNT(*) cnt FROM {bronze_table}
                GROUP BY {pk} HAVING cnt > 1
            ) d
        """)
        n = cursor.fetchone()[0]
        if n > 0:
            issues_found.append({
                "issue_type": "DUPLICATE_RECORDS",
                "column": pk,
                "count": n,
                "percentage": round(n/total_records*100, 1),
                "severity": "HIGH",
                "description": f"{n} duplicate values in {pk}"
            })
            bad_records += n
            print(f"  DUPLICATES in {pk}: {n}")

    # CHECK 5 - Invalid SKU
    for col in columns:
        if 'sku' in col.lower():
            sku_sql = f"""
                SELECT COUNT(*) FROM {bronze_table}
                WHERE {col} IS NOT NULL
                AND NOT REGEXP_LIKE({col}, '^ELF-[0-9]{{5}}$')
            """
            cursor.execute(sku_sql)
            n = cursor.fetchone()[0]
            if n > 0:
                issues_found.append({
                    "issue_type": "INVALID_SKU",
                    "column": col,
                    "count": n,
                    "percentage": round(n/total_records*100, 1),
                    "severity": "HIGH",
                    "description": f"{n} invalid SKU formats in {col}"
                })
                bad_records += n
                print(f"  INVALID SKU in {col}: {n}")

    # Health score
    health_score = round(max(0, (total_records - min(bad_records, total_records)) / total_records * 100), 1)

    # Log alerts
    if issues_found:
        for issue in issues_found:
            cursor.execute(
                "INSERT INTO ALERT_LOG (ALERT_ID, ALERT_TYPE, MESSAGE, SEVERITY, CREATED_AT) VALUES (%s, %s, %s, %s, %s)",
                (str(uuid.uuid4())[:8], issue['issue_type'], f"{bronze_table}: {issue['description']}", issue['severity'], datetime.now())
            )
        conn.commit()

    # AI diagnosis
    issues_text = "\n".join([f"- {i['issue_type']} in {i['column']}: {i['description']}" for i in issues_found]) if issues_found else "No issues"
    prompt = f"""You are DataForge AI Quality Agent for E.L.F Beauty.
Table: {bronze_table}
Records: {total_records} total, {bad_records} with issues
Health Score: {health_score}/100
Issues:
{issues_text}
Write 3 sentences explaining business impact. End with STATUS: HEALTHY/WARNING/CRITICAL."""

    response = groq_client.chat.completions.create(
        messages=[{"role": "user", "content": prompt}],
        model="llama-3.3-70b-versatile",
        max_tokens=200
    )
    ai_diagnosis = response.choices[0].message.content.strip()

    cursor.close()
    conn.close()

    quality_status = "HEALTHY" if health_score >= 90 else "WARNING" if health_score >= 70 else "CRITICAL"
    print(f"\nHealth Score: {health_score}/100 — {quality_status}")
    print(f"Total issues: {len(issues_found)}")
    print(f"\nAI Diagnosis:\n{ai_diagnosis}")

    return {
        "status": "success",
        "run_id": run_id,
        "table": bronze_table,
        "total_records": total_records,
        "issues_found": len(issues_found),
        "bad_records": bad_records,
        "health_score": health_score,
        "quality_status": quality_status,
        "issues": issues_found,
        "ai_diagnosis": ai_diagnosis
    }

if __name__ == "__main__":
    print("=== Testing ELF Orders ===")
    run_quality_checks("BRZ_MYSQL_ELF_ORDERS")
    print("\n=== Testing ELF Inventory ===")
    run_quality_checks("BRZ_MYSQL_ELF_INVENTORY")